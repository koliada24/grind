...
class BooleanQueryParser
{
    ...
    public List<Clause> ParseDNF(string query, HashSet<string> validTerms)
    {
        query = query.Trim();
        ...
        var orParts = SplitByTopLevelOperator(query, "OR");
        ...
        foreach (var part in orParts)
        {
            ...
            var andParts = SplitByTopLevelOperator(trimmedPart, "AND");

            foreach (var literal in andParts)
            {
    ...
    private string ApplyDeMorgansLaw(string query)
    {
        ...
        while (Regex.IsMatch(query, pattern))
        {
            query = Regex.Replace(query, pattern, match =>
            {
                ...
                if (innerExpression.Contains(" AND "))
                {
                    var andParts = SplitByTopLevelOperator(innerExpression, "AND");
                    var transformed = string.Join(" OR ", andParts.Select(p => $"NOT {p.Trim()}"));
                    return transformed;
                }
