class Program
{
    static void Main()
    {
        ...
        Console.WriteLine("Variant: Conjunctive Normal Form (КНФ) - AND of ORs\n");
    ...
    static void ExecuteQueries(InvertedIndex index, List<Document> documents, HashSet<string> validTerms)
    {
        ...
        Console.WriteLine("Enter queries in КНФ (Conjunctive Normal Form)");
        Console.WriteLine("Format: (term1 OR term2) AND (term3 OR NOT(term4)) AND ...");
        ...
        while (true)
        {
            ...
            try
            {
                var clauses = parser.ParseKNF(query, validTerms);
                ...
                for (int i = 0; i < clauses.Count; i++)
                {
                    ...
                    Console.WriteLine($"  Clause {i + 1}: {string.Join(" OR ", literals)}");
                }
